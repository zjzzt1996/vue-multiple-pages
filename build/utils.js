...
// 获取路由文件，用于加到url上面
exports.routerPath = function (modulename) {
  let path = modulename || '*'
  const routeFiles = glob.sync(MOUDLES_PATH + '/' + path + '/router/route.js')
  let arr = []
  routeFiles.forEach(routeFile => {
    const routeFilePath = require(routeFile)
    routeFilePath.route.forEach(path=>{
      arr.push(path.path)
    })
  })
  return arr
