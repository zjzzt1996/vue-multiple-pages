    ...
    portfinder.getPort((err, port) => {
      if (err) {
        ...
        let urls = Object.keys(utils.devEntries(envModule))
          .map(
            i => {
              if (utils.routerPath(i).length > 0) {
                return utils.routerPath(i).map(r => `http://${devWebpackConfig.devServer.host}:${port}/${i}.html#${r}`)
              } else {
                return `http://${devWebpackConfig.devServer.host}:${port}/${i}.html`
              }
            }
          )
          .join('\n\r')
